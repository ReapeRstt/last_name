...
class Add_en():
    def __init__(self):
        def add_to_db():
            ...
            cursor.execute(
                "INSERT INTO legal_entities (entitie_id, en_name, address, phone_number) "
                "VALUES (%s, %s, %s, %s)",
        ...
        id_label = Label(entitie_add, text="ID", font='Arial 11 bold', bg='#fafafa', padx=10, pady=8)
        id_label.pack()
        id_entry = Entry(entitie_add)
        id_entry.pack()
...
class Add_lo():
    def __init__(self):
        def add_to_db():
            ...
            # Выполнение SQL-запроса для добавления данных в таблицу legal_entities
            cursor.execute(
                "INSERT INTO loans (loan_id, entitie_id, amount, percent, term)"
                "VALUES (%s, %s, %s, %s, %s)",
                (lo_id, en_id, amount, percent, term))
        ...
        loan_add['bg'] = '#fafafa'
        loan_add.geometry('350x350')
        loan_add.resizable(width=False, height=False)

        lo_id_label = Label(loan_add, text="ID кредита", font='Arial 11 bold', bg='#fafafa', padx=10, pady=8)
        lo_id_label.pack()
        lo_id_entry = Entry(loan_add)
        lo_id_entry.pack()
    ...
    def __init__(self):
        ...
        def search_en():
            ...
            if en_id == '':
                ...
                # Выполнение SQL-запроса для выборки записей из таблицы
                command1 = "SELECT * FROM legal_entities WHERE entitie_id = {en_id}"
                command2 = command1.format(en_id=en_id)
                cursor.execute(command2)

                # Получение всех выбранных записей
                serched_en = cursor.fetchall()
                if serched_en == []:
                    messagebox.showinfo(title='Результат', message='Записи с таким ID не найдено.')
        ...
        def search_lo():
            ...
            if lo_id == '':
                ...
                # Выполнение SQL-запроса для выборки записей из таблицы
                command1 = "SELECT * FROM loans WHERE loan_id = {lo_id}"
                command2 = command1.format(lo_id=lo_id)
                cursor.execute(command2)
        ...
        def update_en():
            ...
            def update():
                    ...
                    if responce:
                        ...
                        cursor2.execute(
                            "INSERT INTO legal_entities (entitie_id, en_name, address, phone_number) "
                            "VALUES (%s, %s, %s, %s)",
                            (upd_id, upd_name, upd_address, upd_phone))
            ...
            id_label = Label(entitie_upd, text="ID", font='Arial 11 bold', bg='#fafafa', padx=10, pady=8)
            id_label.pack()
            id_entry = Entry(entitie_upd)
            id_entry.pack()
            id_entry.insert(0, en_id)
        ...
        def update_lo():
            ...
            def update():
                    ...
                    if responce:
                        ...
                        cursor2.execute(
                            "INSERT INTO loans (loan_id, entitie_id, amount, percent, term) "
                            "VALUES (%s, %s, %s, %s, %s)",
                            (upd_lo_id, upd_en_id, upd_amount, upd_percent, upd_term))
            ...
            loan_upd['bg'] = '#fafafa'
            loan_upd.geometry('350x350')
            loan_upd.resizable(width=False, height=False)

            lo_id_label = Label(loan_upd, text="ID кредита", font='Arial 11 bold', bg='#fafafa', padx=10, pady=8)
            lo_id_label.pack()
            lo_id_entry = Entry(loan_upd)
            lo_id_entry.pack()
            lo_id_entry.insert(0, lo_id)
